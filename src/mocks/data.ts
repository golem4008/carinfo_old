...
const carCompanies: CarCompany[] = Array.from(uniqueCompanies).map((company, index) => {
  ...
  switch (company) {
   ...
   return {
    ...
    logo: logoUrl,
    // 使用预定义颜色或生成随机颜色
    color: companyColors[company] || '#' + Math.floor(Math.random() * 16777215).toString(16)
  };
});
...
export const getCurrentCarModelData = (dateRange?: DateRange): CarModelData[] => {
  return filterDataByDateRange(staticCarModelData, dateRange);
};
...
export const getMarketShareData = (dateRange?: DateRange): MarketShareData[] => {
  ...
  // 计算市场份额并生成数据
   return Object.keys(totalSalesByCompany).map((company, index) => {
    ...
    return {
      company,
      share: totalSales > 0 ? parseFloat(((totalSalesByCompany[company] / totalSales) * 100).toFixed(1)) : 0,
      color: companyData?.color || '#' + Math.floor(Math.random()*16777215).toString(16)
    };
